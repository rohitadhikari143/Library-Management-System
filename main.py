...
#creating table
        ...
        self.listTree.place(x=320,y=318)
        self.vsb.place(x=1028,y=319,height=305)
            ...
            if(len(self.studid.get())==0):
             ...
             try:
                ...
                cursor.execute("Select bi.issue_id,s.name,b.name,bi.issue_date,bi.return_date from issue_book bi,"
                               "student s,book b where s.stud_id = bi.stud_id and b.book_id = bi.book_id and "
                               "s.stud_id = %s",[change])
        ...
        def ent():
            if (len(self.bookid.get()) == 0):
             ...
             try:
                ...
                self.myCursor.execute("Select bi.issue_id,s.name,b.name,bi.issue_date,bi.return_date from issue_book bi,"
                                      "student s,book b where s.stud_id = bi.stud_id and b.book_id = "
                                      "bi.book_id and b.book_id = %s",[book])
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    self.label3.place(x=360, y=15)

                    self.label4 = Label(self, text="ENTER STUDENT",bg="gray", font=('Arial', 18, 'bold'))
                    self.label4.place(x=150, y=89)

                    self.studid = Entry(self, textvariable=self.a, width=90)
                    self.studid.place(x=405, y=95)

                    self.srt = Button(self, text='Search', width=15, font=('arial', 10),command = ser).\
                        place(x=1000, y=93)

                    self.label5 = Label(self, text="ENTER THE BOOK ID",bg="gray", font=('Arial', 18, 'bold'))
                    self.label5.place(x=110, y=153)

                    self.bookid = Entry(self, textvariable=self.b, width=90)
                    self.bookid.place(x=405, y=160)

                    self.brt = Button(self, text='Find', width=15,
                                      font=('arial', 10),command = ent).place(x=1000, y=157)

                    self.label6 = Label(self, text="INFORMATION DETAILS",bg="gray",
                                        font=('Courier new', 15, 'underline', 'bold'))
                    self.label6.place(x=550, y=280)

                    self.button = Button(self, text='Search Student', width=25,
                                         font=('Courier new', 10), command=sest).place(x=220,y=230)

                    self.button = Button(self, text='Search Book', width=25,
                                         font=('Courier new', 10), command=sea).place(x=500,y=230)

                    self.brt = Button(self, text="Issue Book", width=15,
                                      font=('Courier new', 10), command=ib).place(x=780, y=230)

                    self.brt = Button(self, text="Return Book", width=15,
                                      font=('Courier new', 10), command=ret).place(x=980, y=230)

                    self.brt = Button(self, text="LOGOUT", width=15,bg="red",
                                      font=('Courier new', 10), command=log).place(x=1140, y=120)
